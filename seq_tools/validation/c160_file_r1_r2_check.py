    ...
    def check(self):
        ...
        for rg in self.metadata.get('read_groups'):
            if 'is_paired_end' not in rg:
                message = "Required field 'is_paired_end' is not found in metadata JSON"
                self.logger.info(message)
                self.message = message
                self.status = 'INVALID'
                return
            ...
            if not isinstance(rg['is_paired_end'], bool):
                message = "Required field 'is_paired_end' should be Boolean type."
                self.logger.info(message)
                self.message = message
                self.status = 'INVALID'
                return
            ...
            if rg['is_paired_end']:
                if not 'file_r1' in rg or not rg['file_r1']:
                    message = "Required field 'file_r1' is not found in metadata JSON for paired end sequencing reads."
                    self.logger.info(message)
                    self.message = message
                    self.status = 'INVALID'
                    return
                if not 'file_r2' in rg or not rg['file_r2']:
                    message = "Required field 'file_r2' is not found in metadata JSON for paired end sequencing reads."
                    self.logger.info(message)
                    self.message = message
                    self.status = 'INVALID'
                    return
                if rg['file_r1'].endswith('.bam') and not rg['file_r1'] == rg['file_r2']:
                    message = "Fields 'file_r1' and 'file_r2' should be the same for paired end BAM sequencing reads."
                    self.logger.info(message)
                    self.message = message
                    self.status = 'INVALID'
                if not rg['file_r1'].endswith('.bam') and rg['file_r1'] == rg['file_r2']:
                    message = "Fields 'file_r1' and 'file_r2' should NOT be the same for paired end FASTQ sequencing reads."
                    self.logger.info(message)
                    self.message = message
                    self.status = 'INVALID'
                ...
                if 'file_r2' in rg and rg['file_r2']:
                    message = "Field 'file_r2' should NOT been populated in metadata JSON for single end sequencing reads."
                    self.logger.info(message)
                    self.message = message
                    self.status = 'INVALID'
                    return 
        ...
        if not fns == fls:
            message =  "The files listed in 'read_groups' section are %s. They do NOT match the files specified in files section: %s." % \
                (','.join(fns), ','.join(fls))
            self.message = message
            self.status = 'INVALID'
            self.logger.info(message)
