...
- Verifies each read group record has an unique `submitter_read_group_id` value
...
- Verifies `submitter_read_group_id` per read group conforms to regex: 
```
^[a-zA-Z0-9\\-_:\\.]+$
```
...
- Verifies each `read_group` record has an unique `platform_unit` value
...
- If `is_paired_end==True` has `file_r1` and `file_r2`.
- If `is_paired_end==False` has only `file_r1`
...
- Verifies each entry in `file` section is uniquely named
...
- Verifies each entry in the `file` section is mentioned in `read_groups` section and no additional files exist
...
- Verifies each `read_group` record has an unique `read_group_id_in_bam` value
...
- Verifies each `file` consists of only basename and no path 
...
- Verifies `read_group` `ID`s in BAM header exist, are unique and conform to the following regex:
```
^[0-9a-zA-Z-_:\'\.\+]+$
```
...
- Verifies `file`'s `size` (bytes) matches size in metadata payload
...
- Verifies `file`'s `Md5sum` matches Md5 in metadata payload
